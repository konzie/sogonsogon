...
<jsp:include page="../common/header.jsp"/>
	<div class="container">
		<div class="container pb-5 mb-5" id="content-main">
			...
			<form action="updateAction?cp=${param.cp}" method="post" 
				   role="form" onsubmit="return validate();" enctype="multipart/form-data">
			<!-- enctype="multipart/form-data" -->
				...
				<div class="form-inline mb-2">
					...
					<input type="text" class="form-control" id="title" name="roomBoardTitle" size="70"
						value="${board.roomBoardTitle }">
				</div>
					...
					<div class="boardImg" id="titleImgArea">
						<img id="titleImg" width="200" height="200"
							<%-- titleImgSrc 변수가 만들어진 경우 == 기존 이미지 중 썸네일 이미지가 있는 경우
								 -> img 태그의 src속성을 추가하고 속성값으로 titleImgSrc의 값을 작성 하겠다.
							 --%>
				...
				<div class="form-group">
					...
					<textarea class="form-control" id="content" name="roomBoardContent"
						rows="10" style="resize: none;">${board.roomBoardContent }</textarea>
				</div>
	...
	<script>
		...
		// 이미지 삭제 버튼의 수 만큼 배열 요소 만들고 모두 false로 초기화  
		...
		$(".deleteImg").on("click", function(event){
			// event 매개변수 : 이벤트와 이벤트가 발생한 객체에 대한모든 정보가 담겨있음.
			event.stopPropagation(); // 삭제 버튼 -> 이미지 클릭 -> file태그 클릭 == 이벤트 버블링
			                         // 이벤트 버블링을 멈춤(삭제)
			...
			var $img = $("<img>", {id : $el.attr("id"), width : $el.css("width"),
			...
			// inpuut type="file" 태그에 있는 value값을 초기화
		...
		$.each($("#category>option"), function(index, item){
			if($(item).text() == "${board.roomBoardType}"){
		...
		function validate() {
			...
			for (var i = 0; i < deleteImages.length; i++) {
				var $deleteImage = $("<input>", {type : "hidden", name : "deleteImages",
